...
let rec convert_instr fun_name exit_label entry_label formal_regs label (rtl_instr:Rtltree.instr) =
  ...
  | Egoto l -> Egoto l
  | Ecall (r, id, r_list, l) -> treat_call r id fun_name r_list l exit_label entry_label formal_regs
  in
...
and treat_call r id fun_name r_list l exit_label entry_label formal_regs=
  ...
  if (l<>exit_label || id <> fun_name) then
    ...
    assert(!index_param = nb_args-1);
    instruction_fold
  (* Terminal call in recursive function *)
  else
    let goto_instr = Egoto(entry_label) in
    let aux prev_instr new_r old_r  =
      let prev_label = generate(prev_instr) in 
      Embinop(Ops.Mmov, new_r, old_r, prev_label)
    in
    List.fold_left2 aux goto_instr r_list formal_regs;;



let allocate_activation_table formal_registers new_regs nb_args entry_label =
  ...
  for i = 1 to k do
    label_retrieve := generate (Embinop(Ops.Mmov, List.nth Register.parameters (i-1), List.nth formal_registers (i-1), !label_retrieve));
  done;

  if(nb_args > k) then begin
    ...
    for i = k to nb_args-1 do
      ...
      label_retrieve := generate (Eget_param((i-k+2)*word_size, List.nth formal_registers i, !label_retrieve))
    done;
  end;
...
let convert_graph rtl_graph nb_args formal_registers register_result entry_label exit_label fun_name =
  let new_regs = Hashtbl.create 16 in
  let label_allocate, label_get_formals = allocate_activation_table formal_registers new_regs nb_args entry_label in
  let label_delete = delete_activation_table new_regs register_result in
  Label.M.iter (convert_instr fun_name exit_label entry_label formal_registers) rtl_graph;
  label_allocate, label_delete;;



let convert_rtl (fun_def:Rtltree.deffun) =
  let exit_label = fun_def.fun_exit; in
  let nb_args = List.length fun_def.fun_formals in
  (*let local_variables = Register.S.elements fun_def.fun_locals in*)
  let label_entry, label_delete = convert_graph fun_def.fun_body nb_args fun_def.fun_formals fun_def.fun_result fun_def.fun_entry exit_label fun_def.fun_name in
  associate exit_label (Egoto label_delete);
  {
